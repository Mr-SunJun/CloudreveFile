...
func (fs *FileSystem) Rename(ctx context.Context, dir, file []uint, new string) (err error) {
	...
	err = userService.CheckAction("update", fs.User)
	if err != nil {
		return err
	}
...
func (fs *FileSystem) Copy(ctx context.Context, dirs, files []uint, src, dst string) error {
	...
	err = userService.CheckPerm(dst, fs.User)
	if err != nil {
		return err
	}
	// 获取目的目录
...
func (fs *FileSystem) Delete(ctx context.Context, dirs, files []uint, force, unlink bool) error {
	...
	err := perm.CheckAction("delete", fs.User)

	if err != nil {
		return err
	}
	errCreate := perm.CheckAction("create", fs.User)
	if errCreate != nil {
		return err
	}

	for _, fileTar := range fs.FileTarget {
		folder := model.Folder{

			//OwnerID: service.MasterId,
		}
		folder.ID = fileTar.FolderID
		notFound := folder.Find()
		notFound = folder.TraceRoot()
		if notFound != nil {
			return notFound
		}
		err := perm.CheckPerm(path.Join(folder.Position, folder.Name), fs.User)
		if err != nil {
			return err
		}
