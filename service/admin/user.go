...
func (service *UserService) CheckPerm(currentPath string, user *model.User) *serializer.AppError {
...
func (service *AddUserService) Add() serializer.Response {
	if service.User.ID > 0 {
		...
		user.Dirs = service.User.Dirs
		user.Action = service.User.Action
		// 检查愚蠢操作
