...
func (folder *Folder) GetChildFolder() ([]Folder, error) {
	...
	if result.Error == nil {
		for i := 0; i < len(folders); i++ {
			DB.Model(&Folder{}).Where("parent_id = ?", folders[i].ID).Count(&count)
			folders[i].IsEmpty = strconv.Itoa(If(count > 0, 1, 0))
...
func GetRecursiveChildFolder(dirs []uint, uid uint, includeSelf bool) ([]Folder, error) {
	...
	//result := DB.Where(" id in (?)", uid, dirs).Find(&parFolders)
	...
	for i := 0; i < 65535; i++ {
		...
		result = DB.Where("parent_id in (?)", uid, parentIDs).Find(&parFolders)
...
func GetFoldersByIDs(ids []uint, uid uint) ([]Folder, error) {
	...
	result := DB.Where("id in (?) ", ids, uid).Find(&folders)
	return folders, result.Error
}
