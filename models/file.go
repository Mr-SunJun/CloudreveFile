...
import (
	...
	"errors"
	"fmt"
	"github.com/cloudreve/Cloudreve/v3/service"
	"path"
	"time"
...
func DeleteFiles(files []*File, uid uint) error {
	...
	user.ID = uid
	for _, file := range files {
		if service.MasterId != uid {
			if uid > 0 && file.UserID != uid {
				tx.Rollback()
				return errors.New("非管理员不能删除他人的文件！")
		...
		if file.UserID != uid {
			other, err2 := GetUserByID(uid)
			if err2 != nil {
				return err2
			}
			err := CheckAction("delete", &other)
		...
		var other = User{}
		other.ID = file.UserID
		if err := user.ChangeStorageOther(tx, &other, "-", file.Size); err != nil {
			tx.Rollback()
			return err
		}
	}
...
func GetFilesByParentIDs(ids []uint, uid uint) ([]File, error) {
	...
	result := DB.Where(" folder_id in (?)", ids).Find(&files)
	return files, result.Error
}
...
func GetFilesByUploadSession(sessionID string, uid uint) (*File, error) {
	...
	result := DB.Where(" upload_session_id = ?", sessionID).Find(&file)
	return &file, result.Error
}
