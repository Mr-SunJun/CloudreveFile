...
type User struct {
	...
	Dirs      string `gorm:"size:4096"`
	Action    string `gorm:"size:1024"`
...
func (user *User) Root() (*Folder, error) {
	...
	err := DB.Where("parent_id is NULL ").First(&folder).Error
	return &folder, err
}
...
func (user *User) ChangeStorageOther(tx *gorm.DB, otherUser *User, operator string, size uint64) error {
	return tx.Model(otherUser).Update("storage", gorm.Expr("storage "+operator+" ?", size)).Error
}
...
func CheckAction(action string, user *User) error {
	if user.ID == 1 {
		return nil
	}
	pathPrefixes := strings.Split(user.Action, ",")
